...
//escreva um código para fazer invisibleBlockGroup (grupo de bloco invisível) colidir com o fantasma, destruir o fantasma e mudar o estado do jogo para end.
    ...
    if(invisibleBlockGroup.isTouching(ghost) || ghost.y > 600 || bombasGroup.isTouching(ghost)){
      ghost.destroy();
      gameState = "end";
    }
...
function bombas(){
if(frameCount % 400 === 0){
  var bomba = createSprite(200,50);
  bomba.addImage("bomba",bombaImg);
  bomba.scale=0.01;
  bomba.velocityY=3;
  bomba.x = Math.round(random(120,400));
  bomba.lifetime=600;
  bombasGroup.add(bomba);
